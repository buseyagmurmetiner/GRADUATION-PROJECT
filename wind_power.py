...
def spend_missing_data(dataset,missing_index):
    ...
    yest_index=(missing_index + 1)-(24*6)
...
for i in range (len(dataset)):
    ...
    if otherDate !=(dataDate+timedelta(minutes=10)):
        dataset=append_missing_data(dataset,i)
    dataset.to_csv('T1_clean.csv')
    #spliting dataset
from numpy import array_split,array,split,asarray

dataset=pd.read_csv('T1_clean.csv')
dataset.head()
...
def split_dataset(data,split_type !=24):
    ...
    train,test=data[1:43201],data[43201:50977]
...
def show_scores(score,scores):
    ...
    #dataset=pd.read_csv('T1_clean.csv',header=0,infer_datetime_format=True,parse_dates=['Date/Time'])
dataset=pd.read_csv('Dataset-1_for_Active_Power_Prediction.csv',header=0)
...
columns=list()
for i in range(72):
    columns.append(start_date + timedelta(minutes=10*i))
